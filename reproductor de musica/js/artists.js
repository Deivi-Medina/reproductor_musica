...
if (!state.followedArtists) {
  state.followedArtists = JSON.parse(localStorage.getItem("mg_followed_artists")) || [];
}

export function openArtistProfile(artistName) {
    ...
    <div class="artist-profile-container" style="max-width: 1200px; margin: 0 auto; padding: 1rem;">
      ...
      <div class="artist-profile-banner" style="position: relative; padding: 4rem 2rem 2rem; border-radius: 24px; overflow: hidden; margin-bottom: 2rem;">
        ...
        <div class="artist-banner-content" style="position: relative; z-index: 3; display: flex; flex-direction: column; gap: 0.5rem; align-items: flex-start;">
          ...
          <h1 class="artist-profile-name" style="font-size: 3rem; font-weight: 800; color: var(--text-primary); letter-spacing: -1px; margin: 0.5rem 0;">${artistName}</h1>
  ...
  if (btnFollow) {
    btnFollow.addEventListener("click", () => {
      if (state.followedArtists.includes(artistName)) {
      ...
      localStorage.setItem("mg_followed_artists", JSON.stringify(state.followedArtists));
    });
  }
    ...
    if (artistAlbums.length === 0) {
      ...
      artistAlbums.forEach((album) => {
        ...
        card.innerHTML = `
          ...
          <div class="card-info">
            <h3>${album.title}</h3>
            <p>${album.genre || "Sin género"}</p>
          </div>
        `;
        card.addEventListener("click", () => openAlbumView(globalIdx));
